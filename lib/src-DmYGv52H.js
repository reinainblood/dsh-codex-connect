...
import { Agent, Dispatcher, ProxyAgent, fetch as fetch$1, getGlobalDispatcher, setGlobalDispatcher } from "undici";
...
*/
function openAICodexProviderWithAstra() {
...
const OPENAI_CODEX_VERIFIED_COMPATIBILITY_URL = "https://raw.githubusercontent.com/reinainblood/dsh-codex-connect/codex/dsh-desktop-2.0.4-alpha426/verified-compatibility.json";
...
function inheritedOriginal(session, assetId) {
	...
	const events = typeof snapshotEvents === "function" ? Reflect.apply(snapshotEvents, session, []) : Reflect.get(session, "events");
	const inheritedEventCount = Reflect.get(session, "inheritedEventCount");
	const seedLength = Reflect.get(session.header, "seedLength");
	const boundary = typeof inheritedEventCount === "number" ? inheritedEventCount : typeof seedLength === "number" ? seedLength : 0;
...
const CODEX_CONNECT_VERSION = "0.1.0-alpha.4.26";
//#endregion
...
async function diagnoseOpenAICodex(options = {}) {
	...
	if (compatibility.status === "incompatible") hints.push("Compatibility mismatch: install the declared DSH plugin API versions with @earendil-works/pi-ai 0.84.3, then run doctor again; no files are changed automatically.");
...
function resolveAutoReviewAction(request) {
	...
	const snapshotEvents = Reflect.get(request.agent.session, "snapshotEvents");
	const calls = (typeof snapshotEvents === "function" ? Reflect.apply(snapshotEvents, request.agent.session, []) : Reflect.get(request.agent.session, "events")).filter((event) => event.type === "tool/call" && event.data.callId === request.callId);
...
//#endregion
export { OPENAI_CODEX_LOCAL_PROXY_CANDIDATES as $, assertNoOpenAICodexProviderConflict as A, logoutOpenAICodex as At, COMPATIBILITY_PACKAGES as B, OPENAI_CODEX_TRANSPORT_ERROR_CODES as C, isFastModeSessionId as Ct, isOpenAICodexTransportError as D, parseOpenAICodexUsage as Dt, OpenAICodexTransportError as E, OPENAI_CODEX_USAGE_URL as Et, checkForOpenAICodexUpdate as F, OPENAI_CODEX_AUTH_FILENAME as Ft, SUPPORTED_NODE_RANGE as G, DSH_PLUGIN_API_PACKAGES as H, compareOpenAICodexVersions as I, OPENAI_CODEX_AUTH_V1_BACKUP_SUFFIX as It, detectCompatibility as J, SUPPORTED_PI_AI_RANGE as K, parseOpenAICodexUpdateResult as L, OPENAI_CODEX_PROVIDER as Lt, openAICodexConflictMessage as M, openAICodexModelCatalog as Mt, CODEX_CONNECT_VERSION as N, OPENAI_CODEX_ACCOUNT_LIMIT as Nt, VIEW_IMAGE_TOOL_NAME as O, readOpenAICodexRateLimits as Ot, OPENAI_CODEX_UPDATE_PATH as P, OPENAI_CODEX_AUTH_DOCUMENT_LIMIT as Pt, OPENAI_CODEX_PROXY_TEST_PATH as Q, parseOpenAICodexVersion as R, OpenAICodexCredentialStore as Rt, OPENAI_CODEX_TRANSPORT_API_VERSION as S, OPENAI_CODEX_FAST_MODE_MAX_SESSION_ID_LENGTH as St, OpenAICodexTransport as T, normalizeTrustedOrigin as Tt, PI_AI_PACKAGE as U, COMPATIBILITY_SCHEMA_VERSION as V, SUPPORTED_DSH_PLUGIN_API_VERSION as W, readInstalledPackageVersion as X, evaluateCompatibility as Y, OPENAI_CODEX_PROXY_DETECT_PATH as Z, OPENAI_CODEX_IMAGE_MAX_COUNT as _, resolveOpenAICodexProxyUrl as _t, name as a, listOpenAICodexProxyCandidates as at, OPENAI_CODEX_IMAGE_PROMPT_MAX_LENGTH as b, FastModeRegistry as bt, migrateOpenAICodexSearchHistory as c, DEFAULT_OPENAI_CODEX_SEARCH_MAX_OUTPUT_TOKENS as ct, OPENAI_CODEX_BASE_URL as d, DEFAULT_OPENAI_CODEX_SETTINGS as dt, OPENAI_CODEX_PROXY_CANDIDATE_LIMIT as et, OPENAI_CODEX_SEARCH_PROVIDER as f, OPENAI_CODEX_SETTINGS_NAMESPACE as ft, OPENAI_CODEX_IMAGE_GENERATION_URL as g, normalizeOpenAICodexProxyUrl as gt, mapOpenAICodexSearchResponse as h, isValidOpenAICodexProxyUrl as ht, inject as i, detectOpenAICodexProxies as it, diagnoseOpenAICodex as j, openAICodexAuthStatus as jt, IMAGE_GENERATE_TOOL_NAME as k, loginOpenAICodex as kt, CODEX_AUTO_REVIEW_MODEL as l, DEFAULT_OPENAI_CODEX_SEARCH_MODE as lt, OpenAICodexSearchProvider as m, isValidOpenAICodexContextWindowOverrides as mt, OPENAI_CODEX_SETTINGS_NS as n, OPENAI_CODEX_PROXY_PROBE_URL as nt, OPENAI_CODEX_HISTORY_BACKUP_SUFFIX as o, DEFAULT_OPENAI_CODEX_PROXY_URL as ot, OPENAI_CODEX_SEARCH_URL as p, decodeOpenAICodexSettings as pt, assessCompatibility as q, apply as r, OpenAICodexProxyManager as rt, OPENAI_CODEX_SEARCH_MODEL_REQUEST_EVENT as s, DEFAULT_OPENAI_CODEX_SEARCH_CONTEXT_SIZE as st, Config as t, OPENAI_CODEX_PROXY_PROBE_TIMEOUT_MS as tt, probeCodexAutoReview as u, DEFAULT_OPENAI_CODEX_SEARCH_MODEL as ut, OPENAI_CODEX_IMAGE_MAX_ERROR_BYTES as v, resolveOpenAICodexSettings as vt, OPENAI_CODEX_TRANSPORT_SERVICE as w, OpenAICodexTrustedOriginsStore as wt, OPENAI_CODEX_IMAGE_REQUEST_TIMEOUT_MS as x, OPENAI_CODEX_FAST_MODE_MAX_SESSIONS as xt, OPENAI_CODEX_IMAGE_MAX_RESPONSE_BYTES as y, OPENAI_CODEX_FAST_MODE_PATH as yt, COMPATIBILITY_CONTRACT as z, openAICodexAuthPath as zt };
