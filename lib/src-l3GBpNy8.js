...
//#endregion
//#region src/json-equality.ts
/** Compare JSON-compatible values without depending on a host-version utility package. */
function deepEqualJson(left, right) {
	if (Object.is(left, right)) return true;
	if (Array.isArray(left) || Array.isArray(right)) return Array.isArray(left) && Array.isArray(right) && left.length === right.length && left.every((value, index) => deepEqualJson(value, right[index]));
	if (typeof left !== "object" || left === null || typeof right !== "object" || right === null) return false;
	const leftRecord = left;
	const rightRecord = right;
	const leftKeys = Object.keys(leftRecord).sort();
	const rightKeys = Object.keys(rightRecord).sort();
	return leftKeys.length === rightKeys.length && leftKeys.every((key, index) => key === rightKeys[index] && deepEqualJson(leftRecord[key], rightRecord[key]));
}
//#endregion
//#region src/session-verbosity.ts
/** Selected presets supersede the creation header, which can be stale. */
function resolveSessionVerbosity(session) {
	...
	for (let index = events.length - 1; index >= 0; index--) {
		...
		if (typeof data.agentPreset === "string") return data.agentPreset === "michael" ? "high" : void 0;
	}
	return session.header.agentPreset === "michael" ? "high" : void 0;
}
//#endregion
...
function withOpenAICodexVerbosity(provider, resolveVerbosity) {
	const streamSimple = provider.streamSimple;
	return {
		...provider,
		streamSimple(model, context, options) {
			const sessionId = options?.sessionId;
			const verbosity = provider.id === "openai-codex" && model.provider === "openai-codex" && sessionId !== void 0 ? resolveVerbosity?.(sessionId) : void 0;
			if (verbosity === void 0) return streamSimple.call(provider, model, context, options);
			const previousOnPayload = options?.onPayload;
			return streamSimple.call(provider, model, context, {
				...
				async onPayload(payload, payloadModel) {
					...
					return {
						...
						text: {
							...text,
							verbosity: text.verbosity === "high" ? "high" : verbosity
						}
					};
				}
			});
...
function requestProvider(provider, fastMode, proxyManager, resolveProxyUrl, resolveVerbosity) {
...
function createOpenAICodexProfile(provider, fastMode, proxyManager, resolveProxyUrl, contextWindowOverrides, resolveVerbosity) {
...
*/
function createOpenAICodexAdapter(credentials, resolveAttachments, fastMode, visibleModelIds, proxyManager, resolveProxyUrl, contextWindowOverrides, resolveVerbosity) {
...
async function readOpenAICodexRequestAuth(store, signal) {
	...
	if (credential?.type !== "oauth" || credential.access !== access || !access || typeof credential.accountId !== "string" || credential.accountId.length === 0) return void 0;
	return {
		access,
...
const SUPPORTED_PI_AI_RANGE = "0.84.3";
...
const CODEX_CONNECT_VERSION = "0.1.0-alpha.4.28.1";
//#endregion
...
const OPENAI_CODEX_SETTINGS_NS = typeof legacySettingsNamespace === "function" ? Reflect.apply(legacySettingsNamespace, DshSettings, [OPENAI_CODEX_SETTINGS_NAMESPACE]) : OPENAI_CODEX_SETTINGS_NAMESPACE;
...
function apply(ctx, config) {
	...
	if (typeof legacyInstallSettingsSection === "function") Reflect.apply(legacyInstallSettingsSection, DshSettings, [
		ctx,
...
//#endregion
export { OPENAI_CODEX_LOCAL_PROXY_CANDIDATES as $, assertNoOpenAICodexProviderConflict as A, loginOpenAICodex as At, COMPATIBILITY_PACKAGES as B, openAICodexAuthPath as Bt, OPENAI_CODEX_TRANSPORT_ERROR_CODES as C, OPENAI_CODEX_FAST_MODE_MAX_SESSION_ID_LENGTH as Ct, isOpenAICodexTransportError as D, OPENAI_CODEX_USAGE_URL as Dt, OpenAICodexTransportError as E, normalizeTrustedOrigin as Et, checkForOpenAICodexUpdate as F, OPENAI_CODEX_AUTH_DOCUMENT_LIMIT as Ft, SUPPORTED_NODE_RANGE as G, DSH_PLUGIN_API_PACKAGES as H, compareOpenAICodexVersions as I, OPENAI_CODEX_AUTH_FILENAME as It, detectCompatibility as J, SUPPORTED_PI_AI_RANGE as K, parseOpenAICodexUpdateResult as L, OPENAI_CODEX_AUTH_V1_BACKUP_SUFFIX as Lt, openAICodexConflictMessage as M, openAICodexAuthStatus as Mt, CODEX_CONNECT_VERSION as N, openAICodexModelCatalog as Nt, VIEW_IMAGE_TOOL_NAME as O, parseOpenAICodexUsage as Ot, OPENAI_CODEX_UPDATE_PATH as P, OPENAI_CODEX_ACCOUNT_LIMIT as Pt, OPENAI_CODEX_PROXY_TEST_PATH as Q, parseOpenAICodexVersion as R, OPENAI_CODEX_PROVIDER as Rt, OPENAI_CODEX_TRANSPORT_API_VERSION as S, OPENAI_CODEX_FAST_MODE_MAX_SESSIONS as St, OpenAICodexTransport as T, OpenAICodexTrustedOriginsStore as Tt, PI_AI_PACKAGE as U, COMPATIBILITY_SCHEMA_VERSION as V, SUPPORTED_DSH_PLUGIN_API_VERSION as W, readInstalledPackageVersion as X, evaluateCompatibility as Y, OPENAI_CODEX_PROXY_DETECT_PATH as Z, OPENAI_CODEX_IMAGE_MAX_COUNT as _, resolveOpenAICodexProxyUrl as _t, name as a, listOpenAICodexProxyCandidates as at, OPENAI_CODEX_IMAGE_PROMPT_MAX_LENGTH as b, OPENAI_CODEX_FAST_MODE_PATH as bt, migrateOpenAICodexSearchHistory as c, DEFAULT_OPENAI_CODEX_SEARCH_MAX_OUTPUT_TOKENS as ct, OPENAI_CODEX_BASE_URL as d, DEFAULT_OPENAI_CODEX_SETTINGS as dt, OPENAI_CODEX_PROXY_CANDIDATE_LIMIT as et, OPENAI_CODEX_SEARCH_PROVIDER as f, OPENAI_CODEX_SETTINGS_NAMESPACE as ft, OPENAI_CODEX_IMAGE_GENERATION_URL as g, normalizeOpenAICodexProxyUrl as gt, mapOpenAICodexSearchResponse as h, isValidOpenAICodexProxyUrl as ht, inject as i, detectOpenAICodexProxies as it, diagnoseOpenAICodex as j, logoutOpenAICodex as jt, IMAGE_GENERATE_TOOL_NAME as k, readOpenAICodexRateLimits as kt, CODEX_AUTO_REVIEW_MODEL as l, DEFAULT_OPENAI_CODEX_SEARCH_MODE as lt, OpenAICodexSearchProvider as m, isValidOpenAICodexContextWindowOverrides as mt, OPENAI_CODEX_SETTINGS_NS as n, OPENAI_CODEX_PROXY_PROBE_URL as nt, OPENAI_CODEX_HISTORY_BACKUP_SUFFIX as o, DEFAULT_OPENAI_CODEX_PROXY_URL as ot, OPENAI_CODEX_SEARCH_URL as p, decodeOpenAICodexSettings as pt, assessCompatibility as q, apply as r, OpenAICodexProxyManager as rt, OPENAI_CODEX_SEARCH_MODEL_REQUEST_EVENT as s, DEFAULT_OPENAI_CODEX_SEARCH_CONTEXT_SIZE as st, Config as t, OPENAI_CODEX_PROXY_PROBE_TIMEOUT_MS as tt, probeCodexAutoReview as u, DEFAULT_OPENAI_CODEX_SEARCH_MODEL as ut, OPENAI_CODEX_IMAGE_MAX_ERROR_BYTES as v, resolveOpenAICodexSettings as vt, OPENAI_CODEX_TRANSPORT_SERVICE as w, isFastModeSessionId as wt, OPENAI_CODEX_IMAGE_REQUEST_TIMEOUT_MS as x, FastModeRegistry as xt, OPENAI_CODEX_IMAGE_MAX_RESPONSE_BYTES as y, publicAuthError as yt, COMPATIBILITY_CONTRACT as z, OpenAICodexCredentialStore as zt };
