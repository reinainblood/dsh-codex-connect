#!/usr/bin/env node
import { At as logoutOpenAICodex, H as DSH_PLUGIN_API_PACKAGES, Lt as OPENAI_CODEX_PROVIDER, Mt as openAICodexModelCatalog, N as CODEX_CONNECT_VERSION, Rt as OpenAICodexCredentialStore, Tt as normalizeTrustedOrigin, X as readInstalledPackageVersion, Y as evaluateCompatibility, c as migrateOpenAICodexSearchHistory, d as OPENAI_CODEX_BASE_URL, gt as normalizeOpenAICodexProxyUrl, j as diagnoseOpenAICodex, jt as openAICodexAuthStatus, kt as loginOpenAICodex, l as CODEX_AUTO_REVIEW_MODEL, u as probeCodexAutoReview, wt as OpenAICodexTrustedOriginsStore, zt as openAICodexAuthPath } from "./src-DmYGv52H.js";
...
function completedResponse(value, model, streamedText) {
	if (!record(value) || value["status"] !== "completed" || !Array.isArray(value["output"])) return false;
	if (value["model"] !== model) return false;
	return streamedText || value["output"].some((item) => record(item) && item["type"] === "message" && item["role"] === "assistant" && Array.isArray(item["content"]) && item["content"].some((part) => record(part) && part["type"] === "output_text" && typeof part["text"] === "string" && part["text"].trim().length > 0));
}
...
function completedStream(text, model) {
	...
	for (const line of text.split(/\r\n|\r|\n/u)) if (line === "") {
		...
		if (event["type"] === "response.output_text.delta" && typeof event["delta"] === "string" && event["delta"].trim().length > 0) streamedText = true;
...
async function probeCodexResponses(request, createDispatcher = (proxyUrl) => proxyUrl === void 0 ? new Agent() : new ProxyAgent(proxyUrl)) {
	...
	try {
		...
		if (httpStatus !== 200 || response.body === null || contentType !== null && !contentType.toLowerCase().startsWith("text/event-stream")) {
			await response.body?.cancel();
			return {
				outcome: "incomplete",
				httpStatus
...
var CodexCapabilityDiagnostics = class {
	...
	*/
	async inspect(request) {
		...
		const runtime = DSH_PLUGIN_API_PACKAGES.some((name) => versions[name] !== null && versions[name] !== "0.1.2-alpha.1") || compatibility.status === "incompatible" ? result("rejected", "declared-version-mismatch", "Use DSH API 0.1.2-alpha.1 and pi-ai 0.84.3 together. DSH 0.1.0-rc.7 requires Codex Connect 0.1.0-alpha.4.14.") : missing || compatibility.status === "unknown" ? result("unknown", "version-metadata-unavailable", "Run this command from the plugin installation in the intended profile.") : result("supported", "declared-host-versions-match", "Host package versions satisfy the declared requirements; this is not a live profile or browser compatibility test.");
