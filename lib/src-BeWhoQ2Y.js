...
function assertOpenAICodexContextWindowModelIds(overrides, catalog) {
	const ids = new Set(catalog.map((model) => model.id));
	for (const id of Object.keys(overrides ?? {})) if (!ids.has(id)) throw new TypeError(`OpenAI Codex contextWindowOverrides contains unknown model id "${id}"`);
}
/**
...
function registerOpenAICodexAuthRoutes(ctx, store, trustedOriginsOverride, fastModeOverride, proxyManager, resolveProxyUrl) {
...
async function diagnoseOpenAICodex(options = {}) {
	...
	if (compatibility.status === "incompatible") hints.push("Compatibility mismatch: install the declared DSH plugin API versions and pin @earendil-works/pi-ai to 0.84.3, then run doctor again; no files are changed automatically.");
...
function apply(ctx, config) {
	...
	const validateSettings = (value) => {
		...
		assertOpenAICodexContextWindowModelIds(value.contextWindowOverrides ?? void 0, catalog);
	};
