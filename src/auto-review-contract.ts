...
export function resolveAutoReviewAction(request: ApprovalRequestEvent): AutoReviewAction | undefined {
  ...
  const retainedEvents = (
    typeof snapshotEvents === 'function'
      ? Reflect.apply(snapshotEvents, request.agent.session, [])
      : Reflect.get(request.agent.session, 'events')
  ) as ReturnType<Agent['session']['snapshotEvents']>
  const calls = retainedEvents.filter(event => event.type === 'tool/call' && event.data.callId === request.callId)
